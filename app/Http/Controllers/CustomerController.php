...
class CustomerController extends Controller
{
    ...
    public function store(Request $request): RedirectResponse
    {
        try {
            ...
            return redirect()->route('customers.index')->with('success', 'تمت إضافة المشترك بنجاح.');
        } catch (Throwable $e) {
            Log::error('Customer store failed', ['message' => $e->getMessage()]);

            return back()->withErrors(['general' => 'حدث خطأ أثناء حفظ المشترك.'])->withInput();
        }
    }
    ...
    public function update(Request $request, Customer $customer): RedirectResponse
    {
        try {
            ...
            return redirect()->route('customers.index')->with('success', 'تم تعديل بيانات المشترك.');
        } catch (Throwable $e) {
            ...
            return back()->withErrors(['general' => 'حدث خطأ أثناء تعديل بيانات المشترك.'])->withInput();
        }
    }
    ...
    public function topup(Request $request, Customer $customer): RedirectResponse
    {
        try {
            ...
            return back()->with('success', 'تمت إضافة الرصيد بنجاح.');
        } catch (Throwable $e) {
            ...
            return back()->withErrors(['general' => 'حدث خطأ أثناء تعبئة الرصيد.'])->withInput();
        }
    }

    private function validatedData(Request $request, ?int $customerId = null): array
    {
        $meterNumberRule = Rule::unique('customers', 'meter_number');
        if ($customerId !== null) {
            $meterNumberRule->ignore($customerId);
        }

        return $request->validate([
            'name' => 'required|string|max:255',
            'meter_number' => ['required', 'string', 'max:100', $meterNumberRule],
